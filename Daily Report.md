...
- 20：00~22：00：大概知道这个圆脸的动画和后面的圆盘的碰撞应该是使用canvas来实现的，在图书馆找了本讲canvas动画开发的书，看了前面基本的图形绘制。:book:（发现canvas可以解决后面小球碰撞的动画问题）
...
- ~~字体还没引入还不会，准备等后面的游戏部分写完之后在一起引入。~~（会辣已引入）
...
- ~~红球和篮球不能同时拖动。~~
- ~~小球移动太快容易检测不到。~~（把速度的表达式给修改了一下，目前来看没有问题）
...
- 12：05~13：10：吃饭。
- 13：10~ 15：35：发了一会儿呆，然后把小黑球和另外两个球碰撞时的速度的表达式给优化了一下，暂时解决了速度过快碰撞检测失效的问题 。~~发现蓝球和黑球在同一水平线时会两球相互左右碰撞出现死循环。~~（解决）
- 15：40~17：00：把蓝球和黑球在同一水平线上时蓝球的速度给调了一下，问题解决，等做完了再优化一下。
- 17：00~18：15：吃饭。
- 18：30~18：50：跟家人联系。:phone:
- 19：00~19：20：把胜利的结算蒙版给写了。:trophy:
- 19：20~20：30：上Google查了一下滚动条样式的修改，写了个差不多的滚动条。
- 20：40~21：50：难度切换的动画搞了个2D的。



## 10.5

- 10：00~11：20：逛了一下小熊的blog。:bear:
- 11：20~12：30：完善了一下那个滚动条，颜色随`range.value`的值响应式的变化还没啥思路，
- 12：30~13：40：吃饭，周麻婆:rice:
- 13：40~14：20：查了一下如何将难度的数据传递给下一个界面，学了一下如何使用localStorage，把难度参数传递这点给写了。:dart:
- 14：30~15：00：发现了一个小bug：当黑球在下班屏幕时，上面的蓝球会出现抖动的情况，已解决。
- 15：00~16：00:基本功能已经完成，把这个游戏从头到尾又给玩了一遍，看看有没有遗漏,然后拿给小Ray老师看了看。
- 16：00~19：20：看了一会儿《css世界》。:book:
- 19：30~20：50：和朋友出校吃饭，饭前撸了一下店主的猫。:cat2:
- 21：00~21：50：回来改了一下小Ray老师指出的问题，小球速度的问题没找到。
- 22：30~23：50：怀疑是自己电脑帧率太低了，用室友的电脑看了一下，和在我电脑上的速度差不多。



##  10.6

10：00~11：00：把字体引入给学会了。

## 问题列表

|                    目前的问题/代办                     |                     猜测的原因/解决方案                      |
| :----------------------------------------------------: | :----------------------------------------------------------: |
|                ~~难度选择页面的滚动条~~                | `input[type=range]`和`input[type=range]::-webkit-slider-thumb` |
|               难度的显示文字的3d滚动效果               | 暂时的解决方案是先用2D的代替，等后面都写完了再来补充这些细节。 |
|                原生操作dom时的偏移问题                 |        猜测是定位布局的问题，考虑换用另一种布局方案了        |
| ~~小球的移动太快时与红球和蓝球的碰撞检测会失效和抖动~~ | 猜测和小黑球的碰撞检测和速度有关/把速度的赋值的式子给优化了一下。 |
|         在靠近墙边击球时偶尔会把小球打进墙里去         |                              ——                              |
|           使用canvas无法实现画布的自适应大小           |                              ——                              |
|                 滚动条颜色的响应式变化                 |                              ——                              |
|      不同的JavaScript文件间如何传递变量/共用变量       | 目前是使用localStorage，Google的时候发现这一块的知识还挺多的，以后抽时间整理一下。 |
